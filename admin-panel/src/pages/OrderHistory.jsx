...
export default function OrderHistory() {
  ...
  const handleDownloadPdf = async (storyId, title = "storybook") => {
    if (!storyId) {
                    ...
                    <div
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-3 flex flex-wrap justify-end gap-3">
                          <button
                            onClick={() => handleDownloadPdf(item.storyId, item.title)}
                            disabled={!item.canDownloadPdf}
                            title={item.canDownloadPdf ? "Download PDF" : `Cannot download - Story is ${item.storyStatus}`}
                            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition ${
                              item.canDownloadPdf
                                ? "bg-blue-600/20 text-blue-400 border border-blue-600/30 hover:bg-blue-600/30 cursor-pointer"
