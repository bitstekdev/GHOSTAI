...
const { generateStorybookPdf } = require('../pdf/generateStorybookPdf');
...
router.post('/generate-pdf', async (req, res) => {
  ...
  const orientation = (storyData.story && storyData.story.orientation) || 'landscape';
  const title = (storyData.story?.title || 'story').replace(/[^a-z0-9\-_. ]/gi, '');
  ...
  try {
    ...
    pdfStream.on('end', () => {
      fs.unlink(pdfPath, (err) => {
        if (err) console.warn(`Failed to clean up PDF: ${pdfPath}`, err);
      });
    });
    ...
    fs.unlink(pdfPath, (unlinkErr) => {
      if (unlinkErr) console.warn(`Failed to clean up failed PDF: ${pdfPath}`);
    });
